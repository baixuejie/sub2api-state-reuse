...
func main() {
	...
	if err != nil || !bytes.Contains(source, []byte(`const version = "1.0.13"`)) {
		panic("runtime version must match package version 1.0.13")
	}
	...
	f, e := os.Create("state-reuse-1.0.13.s2plugin")
	if e != nil {
		panic(e)
	}
	z := zip.NewWriter(f)
